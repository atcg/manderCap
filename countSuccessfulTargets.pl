...
my $outFile1 = $outFilePrefix . ".HSPcounts.txt";
...
# Print header rows
print $outFH1 "Sample\taveUnder5Counts\taveOver5Counts\taveOver10Counts\taveOver15Counts\taveOver20Counts\tmax100AveUnder5Counts\tmax100AveOver5Counts\tmax100AveOver10Counts\tmax100AveOver15Counts\tmax100AveOver20Counts\n";
print $outFH2 "Sample\tAverageTargetDepthAcrossHSP\tmax100AverageTargetDepth\n";


my %sampleSummaryHash;
foreach my $sample (@samples) {
    ...
    while (my $result = $searchIO->next_result()) {
        ...
        if (scalar(@bases) >= 100) {
            ...
            #print "Average across best HSP for " . $hit->name() . " for individual $sample: $average\n";
            #print "WEIGHTED Average across best HSP for " . $hit->name() . " for individual $sample: " . $average * $weightingsHash{$sample} . "\n";
            my $weightedAverage = $average * $weightingsHash{$sample};
            if ($weightedAverage > 20) {
                $sampleSummaryHash{$sample}{'aveOver20'}++;
                $sampleSummaryHash{$sample}{'aveOver15'}++;
                $sampleSummaryHash{$sample}{'aveOver10'}++;
                $sampleSummaryHash{$sample}{'aveOver5'}++;
                ...
                $sampleSummaryHash{$sample}{'aveUnder5'}++;
            }

            push(@{$sampleSummaryHash{$sample}{'aveWindowDepth'}}, $weightedAverage);        
            ...
            my $max100aveDepth = 0;
            my $maxBaseCounter = 0;
            print "Processing target " . $hit->name() . "for sample " . $sample ."\n";
            foreach my $baseNum (0 .. (scalar(@bases)-100)) {
                ...
                foreach my $innerBase ($bases[$baseNum] .. ($bases[$baseNum]+99)) {
                    if (exists $sampleResultsHash{$hit->name()}{$innerBase}) {
                        ...
                        #print "Depth at base $innerBase: " . $sampleResultsHash{$hit->name()}{$innerBase} . "\n";
                        $baseCounter++;
                    } else {
                        $windowDepth += 0;
                    }
            ...
            push(@{$sampleSummaryHash{$sample}{'max100AveWindowDepth'}}, $weightedMax100AveDepth);   
            
            if ($weightedMax100AveDepth > 20) {
                $sampleSummaryHash{$sample}{'max100AveOver20'}++;
                $sampleSummaryHash{$sample}{'max100AveOver15'}++;
                $sampleSummaryHash{$sample}{'max100AveOver10'}++;
                $sampleSummaryHash{$sample}{'max100AveOver5'}++;
                ...
                $sampleSummaryHash{$sample}{'max100AveUnder5'}++;
            }
        } else {
            #Do nothing--don't process HSPs under 100bp in length
        }
    }
    print $outFH1 "$sample\t$sampleSummaryHash{$sample}{'aveUnder5'}\t$sampleSummaryHash{$sample}{'aveOver5'}\t$sampleSummaryHash{$sample}{'aveOver10'}\t$sampleSummaryHash{$sample}{'aveOver15'}\t$sampleSummaryHash{$sample}{'aveOver20'}\t$sampleSummaryHash{$sample}{'max100AveUnder5'}\t$sampleSummaryHash{$sample}{'max100AveOver5'}\t$sampleSummaryHash{$sample}{'max100AveOver10'}\t$sampleSummaryHash{$sample}{'max100AveOver15'}\t$sampleSummaryHash{$sample}{'max100AveOver20'}\n";
    print $outFH2 "$sample\t" . sum(@{$sampleSummaryHash{$sample}{'aveWindowDepth'}})/@{$sampleSummaryHash{$sample}{'aveWindowDepth'}} . "\t" . sum(@{$sampleSummaryHash{$sample}{'max100AveWindowDepth'}})/@{$sampleSummaryHash{$sample}{'max100AveWindowDepth'}} . "\n";
}
